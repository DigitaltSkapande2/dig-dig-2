Shader "Hidden/Universal Render Pipeline/BokehDepthOfField"
{
    HLSLINCLUDE
        ...
        half FragCoC(Varyings input) : SV_Target
        {
            ...
            float linearEyeDepth = (unity_OrthoParams.w == 1) ? lerp(_ProjectionParams.z, _ProjectionParams.y, depth) : LinearEyeDepth(depth, _ZBufferParams);
            #else
            float linearEyeDepth = (unity_OrthoParams.w == 1) ? lerp(_ProjectionParams.y, _ProjectionParams.z, depth) : LinearEyeDepth(depth, _ZBufferParams);
            #endif
